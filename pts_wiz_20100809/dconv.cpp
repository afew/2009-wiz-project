...
static UINT DCNVnext = 1UL;

static USHORT SinTable[1025] =
{
...
USHORT DCNVrand(USHORT max);
void DCNVinitRand(USHORT seed);
void DCNVshuffleDim(int cnt, USHORT *tbl);
void DCNVshuffleMain(int cnt, USHORT *tbl);
INT MOTsin(int ang);
INT MOTcos(int ang);
SHORT MOTatan(INT posX,INT posY);

static SHORT MOTatanSub(INT posX, INT posY);
...
USHORT DCNVrand(USHORT max)
{
	...
	if(max){
		return ((USHORT)(DCNVnext / 65536) % max);
	}else{
		return 0;
	}
}
...
INT MOTsin(int ang)
{
	...
	switch(deg & 0x0c00){
		...
		case 0x0800 :
			return (0 - (INT)(SinTable[deg - 2048]));
		case 0x0c00 :
			if((deg = 4096 - deg) < 1024){
				return (0 - (INT)(SinTable[deg]));
			}else{
				return (0 - 0x10000L);
			}
	}
...
INT MOTcos(int ang)
{
	...
	switch(deg & 0x0c00){
		case 0x0000 :
			if(deg){
				return ((INT)(SinTable[1024 - deg]));
			}else{
				return (0x10000L);
			}
		case 0x0400 :
			return (0 - (INT)(SinTable[deg - 1024]));
		case 0x0800 :
...
SHORT MOTatanSub(INT posX, INT posY)
{
	INT absX;
	INT absY;
