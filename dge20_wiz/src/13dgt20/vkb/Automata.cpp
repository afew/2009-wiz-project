...
unsigned char Automata::m_KSTABLE1[2350][3] = {
...
unsigned short Automata::m_KSTABLE2[40][4] = {
...
unsigned int Automata::m_Table_for_2_qwerty[] = {
	/* !          "          #          $          %          & */
...
/*------------------------------------------------------------------------------
Function Name :: GetHangul
Return type   :: ...
Return value  :: ...
Input Value   :: ...
Description   :: 입력된 영문 조합을 한글 문자열로 변환해준다.
------------------------------------------------------------------------------*/
void Automata::GetHangul(unsigned char *szInputBuffer, unsigned char *szOutputBuffer)
{
	int hangul_state = 0;
	int iBufferPosition = 0, iOutputPosition = 0;
	int sp = 0, pc = 0 , stack[10] = { 0, };
	int first = 0, middle = 0, last = 0, temp = 0; // 초, 중, 종, 임시
	int rs = 0;
	...
	while(0 != szInputBuffer[iBufferPosition])
	{
		switch(hangul_state)
		{
		case 1:		// 초성 입력중...
			{
				sp = 0;
				if((temp = fcon(szInputBuffer[iBufferPosition])))
				{
					first = temp;
					hangul_state = 2;
				}
				else if((temp = vow(szInputBuffer[iBufferPosition])))
				{
					first  = 1;
					middle = temp;
					push(2);
		...
		case 2:		// 중성 입력중...
			{
				if((temp = vow(szInputBuffer[iBufferPosition])))
				{
					if(In_KS(first, temp, 1))
					{
						push(2);
						middle = temp;
						hangul_state = 3;
					...
					{
						rs = Convert_3_to_KS((unsigned char)first, 2, 1, &szOutputBuffer[iOutputPosition]);
						first  = 1;
						middle = temp;
						sp     = 0;
						push(2);
				...
				else if((temp = fcon(szInputBuffer[iBufferPosition])))
				{
					rs = Convert_3_to_KS((unsigned char)first, 2, 1, &szOutputBuffer[iOutputPosition]);
					sp = 0;
					first = temp;
		...
		case 3:		// 종성 입력중...
			{
				if((temp = lcon(szInputBuffer[iBufferPosition])) && In_KS(first, middle, temp))
				{
					push(1);
					last = temp;
					pc   = szInputBuffer[iBufferPosition];
					hangul_state = 4;
				}
				else if((temp = comvow_qwerty(middle, szInputBuffer[iBufferPosition])) && In_KS(first, temp, 1))
				{
					push(middle);
					middle = temp;
				}
				...
				{
					rs = Convert_3_to_KS((unsigned char)first, (unsigned char)middle, 1, &szOutputBuffer[iOutputPosition]);
					first = temp;
					sp    = 0;
					hangul_state = 2;
				}
		...
		case 4:		// 복합 종성 입력중...
			{
				if((temp = comcon_qwerty(last, szInputBuffer[iBufferPosition])) && In_KS(first, middle, temp))
				{
					push(last);
					last = temp;
					pc   = szInputBuffer[iBufferPosition];
				...
				{
					if(In_KS(fcon(pc), temp, 1))
					{
						rs = Convert_3_to_KS((unsigned char)first, (unsigned char)middle, (unsigned char)pop(), &szOutputBuffer[iOutputPosition]);
						first  = fcon(pc);
						middle = temp;
						sp     = 0;
						push(2);
					...
					{
						rs = Convert_3_to_KS((unsigned char)first, (unsigned char)middle, (unsigned char)last, &szOutputBuffer[iOutputPosition]);
						first  = 1;
						middle = temp;
						sp     = 0;
						push(2);
...
Description   :: 
------------------------------------------------------------------------------*/
int Automata::In_KS(int f, int m, int t)
{
	unsigned char buf[8];
	return Convert_3_to_KS((unsigned char)f, (unsigned char)m, (unsigned char)t, buf) == 2;
}

/*------------------------------------------------------------------------------
Function Name :: Convert_3_to_KS
Return type   :: ...
Return value  :: ...
Input Value   :: ...
Description   :: 
------------------------------------------------------------------------------*/
int Automata::Convert_3_to_KS(unsigned char f, unsigned char m, unsigned char l, unsigned char *des)
{
	unsigned char key[3];
	int hi, lo, mi, result, found;
	...
	key[2] = l;
	lo = 0;
	hi = sizeof(m_KSTABLE1)/3 - 1;
	mi = 0;
	found = 0;
	while(lo + 1 < hi) {
		mi =(lo + hi)/2;
		result = strncmp((char*)m_KSTABLE1[mi],(char*)key, 3);
		if(result == 0) {
			found = 1;
			break;
		}
	...
	if(!found) {
		if(!strncmp((char*)m_KSTABLE1[lo],(char*)key, 3)) {
			found = 1;
			mi = lo;
		}
		if(!strncmp((char*)m_KSTABLE1[hi],(char*)key, 3)) {
			found = 1;
			mi = hi;
		}
	}
	if(!found) {
		for(mi = 0; mi < 40; mi++)
			if(m_KSTABLE2[mi][1] == f && m_KSTABLE2[mi][2] == m &&
				m_KSTABLE2[mi][3] == l) {
					des[0] = (unsigned char)(m_KSTABLE2[mi][0] >> 8);
					des[1] = (unsigned char)m_KSTABLE2[mi][0];
					return 2;	/* found */
			}
	}
	else {
		des[0] = (unsigned char)(mi /(0xff-0xa1) + 0xb0);
		des[1] = (unsigned char)(mi %(0xff-0xa1) + 0xa1);
		return 2;		/* found */
	}
...
Return value  :: ...
Input Value   :: ...
Description   :: 2 벌식에서 영문자 -> 조합형 초성 변환 결과: 초성이 아니면 0
------------------------------------------------------------------------------*/
int Automata::fcon(int c)
{
	/* 초성의 범위 검색 */
	if(c < '"' || c > 'z')
		return 0;

	if(m_Table_for_2_qwerty[c - '!'] & F_C_F)
		return m_Table_for_2_qwerty[c - '!'] & 0xff;
...
Return value  :: ...
Input Value   :: ...
Description   :: 2 벌식에서 영문자 -> 중성 변환 결과: 중성이 아니면 0
------------------------------------------------------------------------------*/
int Automata::vow(int c)
{
	/* 모음의 범위 검색 */
	if(c < 'L' || c > 'y')
		return 0;

	if(m_Table_for_2_qwerty[c - '!'] & F_V)
		return m_Table_for_2_qwerty[c - '!'] & 0xff;
...
Return value  :: ...
Input Value   :: ...
Description   :: 2벌식에서 영문자 -> 받침 변환결과: 받침이 아니면 0
------------------------------------------------------------------------------*/
int Automata::lcon(int c)
{
	...
	if(m_Table_for_2_qwerty[c - '!'] & F_C_L)
...
/*------------------------------------------------------------------------------
Function Name :: comcon_qwerty
Return type   :: ...
Return value  :: ...
Input Value   :: ...
Description   :: 2 벌식에서(현재 받침, 영문자 입력) -> 받침 변환
------------------------------------------------------------------------------*/
int Automata::comcon_qwerty(int k, int c)
{
	switch(k)
	{
	case 2:			/* ㄱ */
...
/*------------------------------------------------------------------------------
Function Name :: comvow_qwerty
Return type   :: ...
Return value  :: ...
Input Value   :: ...
Description   :: 2벌식에서(현재 중성, 영문 입력) -> 중성 변환
------------------------------------------------------------------------------*/
int Automata::comvow_qwerty(int v, int c)
{
	switch(v)
	{
	case 13:			/* ㅗ */
...
void KeyNode::SetCodeNoShift(int Code)
{
	m_Code[0] = Code;
}

void KeyNode::SetCodeShift(int Code)
{
	m_Code[1] = Code;
}

void KeyNode::SetPickArea(int x, int y, int w, int h)
{
	m_PickArea[0] = x;
	m_PickArea[1] = y;
	m_PickArea[2] = w;
...
void KeyNode::SetRectNPEng(int x, int y, int w, int h)
{
	m_KeyRect[0][0][0] = x / 512.0f;
	m_KeyRect[0][0][1] = 1.0F - y / 512.0f;
	m_KeyRect[0][0][2] =(x + w) / 512.0f;
	m_KeyRect[0][0][3] = 1.0F - (y + h) / 512.0f;
}

void KeyNode::SetRectNPEngShift(int x, int y, int w, int h)
{
	m_KeyRect[0][1][0] = x / 512.0f;
	m_KeyRect[0][1][1] = 1.0F - y / 512.0f;
	m_KeyRect[0][1][2] =(x + w) / 512.0f;
	m_KeyRect[0][1][3] = 1.0F - (y + h) / 512.0f;
}

void KeyNode::SetRectNPKor(int x, int y, int w, int h)
{
	m_KeyRect[0][2][0] = x / 512.0f;
	m_KeyRect[0][2][1] =1.0F - y / 512.0f;
	m_KeyRect[0][2][2] =(x + w) / 512.0f;
	m_KeyRect[0][2][3] =1.0F - (y + h) / 512.0f;
}

void KeyNode::SetRectNPKorShift(int x, int y, int w, int h)
{
	m_KeyRect[0][3][0] = x / 512.0f;
	m_KeyRect[0][3][1] =1.0F - y / 512.0f;
	m_KeyRect[0][3][2] =(x + w) / 512.0f;
	m_KeyRect[0][3][3] =1.0F - (y + h) / 512.0f;
}

void KeyNode::SetRectPEng(int x, int y, int w, int h)
{
	m_KeyRect[1][0][0] = x / 512.0f;
	m_KeyRect[1][0][1] =1.0F - y / 512.0f;
	m_KeyRect[1][0][2] =(x + w) / 512.0f;
	m_KeyRect[1][0][3] =1.0F - (y + h) / 512.0f;
}

void KeyNode::SetRectPEngShift(int x, int y, int w, int h)
{
	m_KeyRect[1][1][0] = x / 512.0f;
	m_KeyRect[1][1][1] =1.0F - y / 512.0f;
	m_KeyRect[1][1][2] =(x + w) / 512.0f;
	m_KeyRect[1][1][3] =1.0F - (y + h) / 512.0f;
}

void KeyNode::SetRectPKor(int x, int y, int w, int h)
{
	m_KeyRect[1][2][0] = x / 512.0f;
	m_KeyRect[1][2][1] =1.0F - y / 512.0f;
	m_KeyRect[1][2][2] =(x + w) / 512.0f;
	m_KeyRect[1][2][3] =1.0F - (y + h) / 512.0f;
}

void KeyNode::SetRectPKorShift(int x, int y, int w, int h)
{
	m_KeyRect[1][3][0] = x / 512.0f;
	m_KeyRect[1][3][1] =1.0F - y / 512.0f;
	m_KeyRect[1][3][2] =(x + w) / 512.0f;
	m_KeyRect[1][3][3] =1.0F - (y + h) / 512.0f;
}

int KeyNode::CheckInner(int x, int y)
{
	if(m_PickArea[0] <= x && m_PickArea[1] <= y)
	{
		if(((m_PickArea[0] + m_PickArea[2]) >= x) &&((m_PickArea[1] + m_PickArea[3]) >= y))
		{
			return TRUE;
		}
	}
...
void KeyNode::SetSpecial(int v)
{
	m_bSpecial[0] = v;
}

void KeyNode::SetSpecialShift(int v)
{
	m_bSpecial[1] = v;
}

void KeyNode::SetKorShiftCode(int v)
{
	m_bKorShiftCode = v;
}

void FuntionKey::SetNPRect(int x, int y, int w, int h)
{
	m_KeyRect[0][0] = x / 512.0f;
	m_KeyRect[0][1] =1.0F - y / 512.0f;
	m_KeyRect[0][2] =(x + w) / 512.0f;
	m_KeyRect[0][3] =1.0F - (y + h) / 512.0f;
}

void FuntionKey::SetPRect(int x, int y, int w, int h)
{
	m_KeyRect[1][0] = x / 512.0f;
	m_KeyRect[1][1] =1.0F - y / 512.0f;
	m_KeyRect[1][2] =(x + w) / 512.0f;
	m_KeyRect[1][3] =1.0F - (y + h) / 512.0f;
}

void FuntionKey::ChangePushed()
{
	if(m_pPushed != NULL)
	{
		(*m_pPushed) = !(*m_pPushed);
...
void FuntionKey::SetPickArea(int x, int y, int w, int h)
{
	m_PickArea[0] = x;
	m_PickArea[1] = y;
	m_PickArea[2] = w;
...
int FuntionKey::CheckInner(int x, int y)
{
	if(m_PickArea[0] <= x && m_PickArea[1] <= y)
	{
		if(((m_PickArea[0] + m_PickArea[2]) >= x) &&((m_PickArea[1] + m_PickArea[3]) >= y))
...
void FuntionKey::SetStatePtr(int* v)
{
	m_pPushed = v;
}

int FuntionKey::GetPushed()
{
	if(m_pPushed != NULL)
	{
		return *m_pPushed;
