...
//(E) afew:
char	_g_GalicLua_Msg_[512]={0};


char*	lua_GalicMsg()
{
	return _g_GalicLua_Msg_;
}
...
static void callalert (lua_State *L, int status) {
  if (status != 0) {
	...
	{  /* no _ALERT function; print it on stderr */
	  ...
	  //(E) afew:
      sprintf(_g_GalicLua_Msg_,lua_tostring(L, -2));
...
//::AFEW
LUALIB_API int luaL_pushresult (lua_State *L, int i, const char *filename)
{
	if (i)
	{
		lua_pushboolean(L, 1);
