...
==================================================================================================*/
#ifdef KX_PLATFORM_LINUX
static int g_fdI2C = 0;	//local file descriptor for the /dev/i2c interface
...
#ifdef KX_PLATFORM_LINUX
	if (block == 1){
		status = i2c_smbus_read_i2c_block_data(g_fdI2C, reg, length, (__u8*)data);
		...
		for (i = 0; i < length; i++){
			ret = i2c_smbus_read_byte_data(g_fdI2C, reg + i);
...
#ifdef KX_PLATFORM_LINUX
	res = i2c_smbus_write_byte_data(g_fdI2C, reg, data);
...
int KIONIX_ACCEL_init(void)
{
...
#if 0
	g_fdI2C = open("/dev/i2c-2", O_RDWR);
	if (g_fdI2C == -1){
		g_fdI2C = open("/dev/i2c-1", O_RDWR);
		if (g_fdI2C == -1){
			g_fdI2C = open("/dev/i2c-3", O_RDWR);
			if (g_fdI2C == -1){		
	...
	if (ioctl(g_fdI2C, I2C_SLAVE, KIONIX_ACCEL_I2C_SLV_ADDR) < 0){
	...
	if (ioctl(g_fdI2C, I2C_FUNCS, &funcs) < 0){
