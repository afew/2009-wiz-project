...
//	printf("Frame Size: %d\n", frameSize);
//	printf("Data Size: %d\n", (int)data->data_size);
//	printf("len/frameSize: %d\n", (int)data->data_size/frameSize);
//	printf("-------------------------------\n\n\n");
    ...
    while(!data->killNow && pDevice->Connected)
    {
        ...
        {
            wrote = write(data->fd, WritePtr, len);				// Write buffer to sound device
...
//	printf("\n\n\n oss_open_playback-------------------------------\n\n\n");
    ...
    data->fd = open(driver, O_WRONLY);		// Sound Device
    if(data->fd == -1)
    {
        free(data);
        if(errno != ENOENT)
